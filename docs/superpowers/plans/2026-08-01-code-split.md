# コード分割 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** `index.html` 1ファイル(約2000行、HTML/CSS/JS同居)を、挙動を一切変えずに `style.css` + `js/*.js`(ES Modules、7ファイル)+ `tests/*.test.js` に分割する。

**Architecture:** ネイティブ ES Modules(`<script type="module">`)。ビルドステップなし。ブラウザは `index.html` から `js/main.js` を読み込み、Node は同じ `js/*.js` を `node:test` から直接 import してテストする。

**Tech Stack:** Vanilla JS (ES2020+), HTML5 Canvas, IndexedDB, `node:test` + `node:assert/strict`(追加npm依存なし)。

## Global Constraints

- 見た目・挙動を一切変えないこと(純粋なリファクタ)。新機能・ロジック変更は別プロジェクト。
- ビルドステップなし。GitHub Pages に現在のファイル群をそのまま置くだけで動くこと。
- `data/*.bin` と `manifest.json` は移動しない。
- 自動テストは `node:test`(Node標準)のみ。npm の外部テストランナーは追加しない。
- 設計書: [docs/superpowers/specs/2026-08-01-code-split-design.md](../specs/2026-08-01-code-split-design.md)

---

## 事前情報: 共有状態の移動先

元の `index.html` のトップレベル可変変数は、参照範囲を精査した結果、以下のように再配置する:

| 元の変数 | 参照ファイル(複数) | 移動先 |
|---|---|---|
| `M1`, `frames`, `maCache`, `SLOTS`, `activeSlot` | main / pattern-search / image-extract / render | `js/state.js` の `state` オブジェクトのプロパティに統合(`state.M1` 等) |
| `galleryOpen` | render.js のみ | `js/render.js` 内のローカル `let` のまま |
| `drawPts`, `drawing` | main.js のみ(手書き検索) | `js/main.js` 内のローカル `let` のまま |
| `$`, `getWIN`, `getFWD` | 複数ファイル | `js/state.js` からexport |

依存関係は一方向(循環なし): `state.js` ← `data.js` ← `pattern-search.js` ← `render.js` ← `image-extract.js` ← `main.js`(`storage.js` も `main.js` からのみ利用)。

---

### Task 1: テスト基盤の準備 + style.css の抽出

**Files:**
- Create: `package.json`
- Create: `style.css`
- Modify: `index.html:6-159`(`<style>`ブロックを削除し `<link>` に置換)

**Interfaces:**
- Produces: `node --test tests/` が実行可能になる(Task 3以降で使う)。`style.css` が `index.html` から参照される。
...
`index.html` の 7〜159行目(`<style>` と `</style>` の間の中身)をそのまま `style.css` にコピーする。中身は現状の全CSSルール(`:root{...}` から `.gal-dot.active{...}` まで)で、1文字も変更しない。

- [ ] **Step 3: index.html を編集 — style.css を読み込む**

`<title>FX相場予測サイト</title>` の直後、`<style>...</style>` ブロックを削除し、代わりに以下を追加する:

```html
<link rel="stylesheet" href="style.css">
```

- [ ] **Step 4: ブラウザで見た目が変わっていないことを確認**

`index.html` をブラウザで開き(ローカルサーバー経由。Task 9 で詳しいサーバー起動手順を扱う)、レイアウト・配色が分割前と同じであることを目視確認する。

- [ ] **Step 5: コミット**

```bash
git add package.json style.css index.html
git commit -m "Extract style.css from index.html; add node:test scaffolding"
```
...
**Interfaces:**
- Produces: `$(id)`, `getWIN()`, `getFWD()`, `state`(`{M1, frames, maCache, SLOTS, activeSlot}` を持つオブジェクト)。以降の全モジュールがこれを import する。
...
```bash
git add js/state.js
git commit -m "Add js/state.js: shared mutable state + DOM helpers"
```

---

### Task 3: data.js の作成 + ユニットテスト
...
**Interfaces:**
- Consumes: なし(DOM/state に依存しない純粋関数群)
- Produces: `TFDEF`, `parseCSV(text)`, `parseBin(buf)`, `resample(bars,min)`, `pctSeries(bars,start,len)`, `znorm(a)`, `sdOf(a)`, `dist(a,b)`, `maSeries(bars,n)`, `trendDir(price,ma,maPrev)` — `pattern-search.js` と `main.js` が import する。

- [ ] **Step 1: js/data.js を作成**
...
- [ ] **Step 3: テストを実行して全件通ることを確認**

Run: `node --test tests/data.test.js`
Expected: 全テストが `pass`(0 fail)

- [ ] **Step 4: コミット**

```bash
git add js/data.js tests/data.test.js
git commit -m "Add js/data.js (CSV/bin parsing, resample, stats) with unit tests"
```

---

### Task 4: storage.js の作成
...
**Interfaces:**
- Consumes: なし
- Produces: `idbSaveM1(m1)`, `idbLoadM1()`, `idbClearM1()` — `main.js` が import する。ブラウザの `IndexedDB` に依存するため自動テスト対象外(Task 9 で目視確認)。

- [ ] **Step 1: js/storage.js を作成**
...
git commit -m "Add js/storage.js: IndexedDB persistence for M1 data"
```

---

### Task 5: pattern-search.js の作成 + ユニットテスト

**Files:**
- Create: `js/pattern-search.js`
- Test: `tests/pattern-search.test.js`

**Interfaces:**
- Consumes: `state` from `./state.js`(`state.frames`, `state.maCache`); `maSeries, dist, znorm, pctSeries, sdOf, trendDir` from `./data.js`
- Produces: `HTF`, `getMA25(tfKey)`, `searchTop(bars,q,WIN,FWD,regime,tfKey,mtf)`, `zigzagAbs(bars,rev)`, `atrOf(bars)`, `rangePos(bars,confIdx,p)`, `maSideAt(ma25,confIdx,price)`, `topCtxOK(bars,ma25,confIdx,topP)`, `bottomCtxOK(bars,ma25,confIdx,botP)`, `PATTERNS`, `clampAnchors(pivs,bars)`, `detectPatterns(bars)`, `backtestPattern(pat,tfKey)` — `image-extract.js`・`render.js`・`main.js` が import する。

- [ ] **Step 1: js/pattern-search.js を作成**
...
- [ ] **Step 2: tests/pattern-search.test.js を作成**
...
Run: `node --test tests/pattern-search.test.js`
Expected: 全テストが `pass`(0 fail)

- [ ] **Step 4: コミット**

```bash
git add js/pattern-search.js tests/pattern-search.test.js
git commit -m "Add js/pattern-search.js (similarity search, pattern detection) with unit tests"
```

---

### Task 6: image-extract.js の作成

**Files:**
- Create: `js/image-extract.js`

**Interfaces:**
- Consumes: `state, $, getWIN, getFWD` from `./state.js`; `detectPatterns, searchTop, HTF` from `./pattern-search.js`; `znorm, maSeries, trendDir` from `./data.js`; `drawShot, updateVerdict, updatePatList, updateGallery` from `./render.js`(Task 7 で作成)
- Produces: `loadChartImage(file,F)`, `extractCandles(img,F)`, `shotBars(F)`, `shotTrendDir(F)`, `reanalyzeLower(F)`, `analyzeShot(F)` — `main.js` が import する。

- [ ] **Step 1: js/image-extract.js を作成**
...
```bash
git add js/image-extract.js
git commit -m "Add js/image-extract.js: screenshot candle extraction + shot analysis"
```

---

### Task 7: render.js の作成

**Files:**
- Create: `js/render.js`

**Interfaces:**
- Consumes: `state, $` from `./state.js`; `sdOf` from `./data.js`; `clampAnchors, backtestPattern` from `./pattern-search.js`
- Produces: `drawShot(F)`, `updateVerdict(F)`, `updatePatList(F)`, `updateGallery(F)`, `setTrackHeight(g)`, `galIndex(track)`, `updateDots(g)`, `wireCarousel(g)`, `drawGalleryCanvases(F)`, `drawMiniChart(c,bars,from,WIN,FWD)` — `image-extract.js` が import する(`drawShot, updateVerdict, updatePatList, updateGallery` のみ)。

**Note:** このファイルは Task 6 の `image-extract.js` より import 順としては先に必要(`image-extract.js` がこのファイルを import する)。実装順はどちらが先でも動くが、動作確認は両方揃った Task 8 完了後に行う。

- [ ] **Step 1: js/render.js を作成**

```js
"use strict";
import { state, $ } from "./state.js";
import { sdOf } from "./data.js";
...
git commit -m "Add js/render.js: canvas drawing, verdict/pattern list, similar-chart gallery"
```

---

### Task 8: main.js の作成 + index.html の配線切り替え

**Files:**
- Create: `js/main.js`
- Modify: `index.html:294-2004`(旧 `<script>` を `<script type="module" src="js/main.js">` に置換)

**Interfaces:**
- Consumes: `state, $, getWIN, getFWD` from `./state.js`; `idbSaveM1, idbLoadM1, idbClearM1` from `./storage.js`; `TFDEF, parseCSV, parseBin, resample, znorm` from `./data.js`; `PATTERNS, atrOf, zigzagAbs, getMA25, clampAnchors` from `./pattern-search.js`; `loadChartImage, reanalyzeLower, analyzeShot` from `./image-extract.js`
- Produces: なし(エントリーポイント)。アプリ起動を担う。

- [ ] **Step 1: js/main.js を作成**
...
- [ ] **Step 2: index.html を編集 — main.js を読み込む**

`index.html` の294行目 `<script>` から2004行目 `</script>` までを削除し、代わりに以下を追加する:
...
git commit -m "Add js/main.js (app bootstrap/wiring) and load it from index.html as ES module"
```
...
Run: `node --test tests/`
Expected: `tests/data.test.js` と `tests/pattern-search.test.js` の全テストが `pass`

- [ ] **Step 2: ローカルサーバーでブラウザ確認**

`fetch("data/manifest.json")` は `file://` では動かないため、簡易HTTPサーバーで確認する。

```bash
cd /Users/shunyamada/claude-projects/fx/scalping-lab && python3 -m http.server 8080
```

ブラウザで `http://localhost:8080/` を開く。

- [ ] **Step 3: 目視・操作確認チェックリスト**

- [ ] ページの見た目(配色・レイアウト)が分割前と同じ
- [ ] 起動時に自動でデータ読み込みが始まり、「✅ ○○本の1分足」が表示される
- [ ] いずれかの時間足枠にチャートのスクリーンショットをドロップし、ローソク足が抽出され、予想パス・類似チャートギャラリーが表示される
- [ ] 「①CSVを読み込む」でCSVファイルをドロップしてもデータが読み込める
- [ ] 手書き検索(値動きの形を描いて検索)が動作する
- [ ] `http://localhost:8080/#dev` で開発用パネルが表示され、パターン実例の検索・前後送りができる
- [ ] ブラウザのコンソールにエラーが出ていない(`read_console_messages` 等で確認)

- [ ] **Step 4: 元の挙動と差分がないか、コンソールログで最終チェック**

`mcp__Claude_Browser__read_console_messages` でエラー・警告が無いことを確認する。問題があれば該当タスクに戻って修正し、修正内容を追加コミットする。

- [ ] **Step 5: 最終コミット(問題があった場合のみ)**

```bash
git add -A
git commit -m "Fix issues found during manual verification of the module split"
```
